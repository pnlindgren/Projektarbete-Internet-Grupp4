...
void nextMove(void * pointer)
{
    ...
    while(1)
    {
        if(collisions(sendInfo.ghostRect) == true)
        {
            if(direction == RIGHT)
            {
                direction = LEFT;
                sendInfo.ghostRect.x = sendInfo.ghostRect.x + 4;
            }
            else
            {
                direction = RIGHT;
        ...
        else
        {
            if(direction == RIGHT)
            {
                sendInfo.ghostRect.x = sendInfo.ghostRect.x - 4;    // lista ut senare hur mycket
            }
